...
class AlertWindow(QWidget):
    ...
    def show_alert(self):
        ...
        if alert_box.clickedButton() == yes_button:
            print("Ejecutando AppStartup.exe...")
            self.run_app_startup()  # Ejecutar el archivo AppStartup.exe
            sys.exit()  # Cerrar el script principal después de lanzar AppStartup.exe
        elif alert_box.clickedButton() == no_button:
            print("Saliendo del programa...")
            sys.exit()  # Salir del programa

    def run_app_startup(self):
        """Función para ejecutar el archivo AppStartup.exe en segundo plano"""
        try:
            # Usar Popen para ejecutar el archivo .exe en segundo plano
            subprocess.Popen(["AppStartup.exe"])
            print("AppStartup.exe se ha lanzado correctamente.")
        except FileNotFoundError:
            print("Error: No se encontró el archivo AppStartup.exe")
        except Exception as e:
            print(f"Error al ejecutar AppStartup.exe: {e}")
